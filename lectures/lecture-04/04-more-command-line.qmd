...
:::{.columns}
:::{.column width="50%"}
- A [terminal]{.alert} is the text window you type into. A [shell]{.alert} is the program inside it that reads your commands and runs them
- The command line gives you [speed, power, reproducibility, and portability]{.alert}
- Every [command has the same shape:]{.alert} `command options arguments`
- [Options start with a dash,]{.alert} and they combine: `ls -lah`
...
## Installing software: Homebrew

:::{style="margin-top: 30px; font-size: 22px;"}
:::{.columns}
:::{.column width="52%"}
- [Homebrew](https://brew.sh) is a [package manager]{.alert}: it downloads, installs, and updates command line tools for you
- It works on macOS, Linux, and WSL. Install it by pasting this into your terminal:
...
- After that, installing a tool is one line:

```text
$ brew install jq
==> Fetching downloads for: jq
✔︎ Bottle jq (1.8.2)
==> Pouring jq--1.8.2.arm64_tahoe.bottle.1.tar.gz
🍺  /opt/homebrew/Cellar/jq/1.8.2: 20 files, 1.2MB
```

- `brew list` shows what you have, `brew upgrade` updates everything, `brew uninstall name` removes one
- On Windows WSL and Ubuntu you can also use `apt`: `sudo apt install jq`
:::

:::{.column width="48%"}
:::{style="text-align: center; margin-top: -30px;"}
[![](figures/homebrew-home-2026.png){width="88%"}](#){data-modal-type="image" data-modal-url="figures/homebrew-home-2026.png"}
:::
...
Packages you may meet in this course and beyond:

- `git`, `gh` (GitHub CLI), `wget`, `curl`, `tree`, `tldr`
- `jq` (read JSON), `duckdb`, `sqlite`, `pandoc`, `quarto`, `uv`, `neovim`
:::
:::
:::
:::
...
- For a directory with files in it, use `rm -r` ([recursive]{.alert}). `-r` and `-R` are the same thing for `rm`
- Adding `-f` ([force]{.alert}) never asks and ignores missing files. Double-check the path first
...
- [Careful]{.alert}: `cp` overwrites a file with the same name without asking. Add `-i` to be asked first
...
- `-iname` matches a name, ignoring capitals (case `i`nsensitive):

```{bash, echo=TRUE}
# "." is the current directory. macOS needs a path, Linux assumes "."
...
:::{style="margin-top: 30px; font-size: 25px;"}
- `wc` counts lines, words, and bytes, in that order. `wc -m` counts characters instead
...
The byte count looks high because `wc` also counts the invisible newline `\n` at the end of every line
:::

:::{.callout-note}
Download the `sonnets.txt` file at <https://github.com/danilofreire/datasci350/blob/main/lectures/lecture-04/sonnets.txt>. Click on "Download raw file". Or use `curl` as shown in the code above. [Download curl here](https://curl.se/download.html), or install it with `brew install curl` (macOS, Linux, WSL) or `sudo apt install curl` (Ubuntu and WSL).
:::
...
- Square brackets hold a [set of characters]{.alert}, and match any [one]{.alert} of them
- `[,.?]` means "a comma, or a full stop, or a question mark". `[a-z]` is a range, and `[^a-z]` means "anything except"
- You can put ranges side by side, with no comma between them. `[a-zA-Z0-9]` is every letter, capital or not, plus every digit: `grep "[a-zA-Z0-9]" file.txt`
:::
...
- [Warning]{.alert}: `-i` edits the file in place and [overwrites the original]{.alert}
  ...
  - On macOS, add empty quotes after `-i`: `sed -i '' 's/.../.../g' sonnets.txt`
:::
...
- `-n` with `p` prints only the lines you ask for. Here, lines 80 to 85:

```{bash, echo=TRUE}
sed -n '80,85p' sonnets.txt
```
...
- macOS ships `pico`, which opens when you type `nano`. `brew install nano` gets the real one. WSL usually has it, or `sudo apt install nano`
- More in the [GNU documentation](https://www.nano-editor.org/docs.php)
:::
...
- The first line, `#!/bin/bash`, is the [shebang](https://en.wikipedia.org/wiki/Shebang_(Unix))
- It tells the system which interpreter should run the file
- To write a Python script, the shebang would be `#!/usr/bin/env python3` 
  - `env` looks `python3` up on your `PATH`, so the same file runs wherever Python is installed
:::
...
- The `./` is not decoration, as the shell does not search the current directory unless you point at it
...
- [File permissions](https://www.redhat.com/en/blog/linux-file-permissions-explained) and [user roles](https://ss64.com/bash/chmod.html), which matter the moment you share a machine
- [Environment variables](https://www.digitalocean.com/community/tutorials/how-to-read-and-set-environmental-and-shell-variables-on-linux) and [SSH](https://www.ssh.com/academy/ssh). Both come back in the cloud lectures
- Watching what your machine is doing, with [top](https://ss64.com/bash/top.html) and [htop](https://hisham.hm/htop/)
- Running commands in parallel with [GNU parallel](https://www.gnu.org/software/parallel/parallel_tutorial.html). We return to parallelism in Lecture 21
...
[![Our course repository. Next class you will learn how it works](figures/course-repo.png){width="100%"}](#){data-modal-type="image" data-modal-url="figures/course-repo.png"}
:::
:::
:::
:::
...
# Poem lines start with two spaces, so allow spaces before "and"
grep -n "^ *and" sonnets.txt
```

```{bash, echo=TRUE}
# Find all lines in sonnets.txt that start with "and" and "And"
# You have to include the -i flag to ignore case! (first 10 of 249 lines)
grep -in "^ *and" sonnets.txt | head -n 10
```
