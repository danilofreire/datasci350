...
- They work in most languages and tools, and in the shell through `grep -E`
...
- [Keep every internal path relative]{.alert}. Never write `"/Users/me/data/thing.csv"`, write `"../data/thing.csv"` instead
- An absolute path works on exactly one machine, and often not on that one either after a few months!
:::
...
- Put the setup first (e.g., your `import` lines)
...
- Around 94% of developers used it in [2022](https://survey.stackoverflow.co/2022/). Later surveys stopped asking, because the answer is everyone
...
:::{style="margin-top: 30px; font-size: 24px;"}
- Git is separate software. You need it installed to use it from the command line or VS Code
- It is probably there already. Check where it is and which version you have:
...
- To install or update it on a Mac, use [Homebrew](https://brew.sh/), "the missing package manager for macOS (or Linux)":
...
- Git for Windows (not WSL): see [happygitwithr.com](https://happygitwithr.com/install-git.html)
:::

## Second step: introduce yourself to Git

:::{style="margin-top: 30px; font-size: 23px;"}
- Git labels every commit with your name and email, so others know it came from you. This matters most when you use Git without GitHub
- Check your current settings (opens a pager, `q` quits):
...
- The name can be your GitHub user name, but does not have to be. The email [should match your GitHub account]{.alert}
- Name the first branch `main`. Otherwise Git still uses `master`, and later commands in this lecture fail:

```{verbatim}
git config --global init.defaultBranch main
```

- More options: [Software Carpentry's setup guide](https://swcarpentry.github.io/git-novice/02-setup.html)
:::
...
- `git commit`: commits (saves) changes
- `git push`: pushes (uploads) changes
- `git pull`: pulls (downloads) changes
- `git status`: shows the status of the working directory
- `git diff`: shows what changed, line by line
- `git log`: shows the commit history
- `git branch`: shows the branches in the repository
- `git checkout`: switches branches (newer Git also has `git switch`)
...
3. I will check where I am with `pwd` 
...
## Adding and removing files from the repo
...
- We deleted `04-visualisation.py` but never staged the deletion, so the file is still part of this commit
- That is why it comes back when we travel to this commit later
:::
...
- Git warns you about a [detached HEAD]{.alert}. That is normal here: you are looking at an old commit, not working on a branch
