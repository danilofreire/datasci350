...
:::{.column width="48%"}
[What the quiz looks like]{.alert}

- Quiz 01 is [next class]{.alert}, in this room, and it is worth 6%
...
- Open notes, open slides, open web, AI allowed (but not encouraged). Say which AI you used, and how it helped you
- [Some students will be selected at random to explain your solutions.]{.alert} If you cannot explain your own work, you will lose points
:::
:::
:::
...
- If a command fails, [put your hand up]{.alert}. Somebody else in the room has the same error. I'm always happy to help 😊
:::
...
The quiz ends with a [push to GitHub]{.alert}. Make sure your machine is logged in before the quiz:
...
:::{.columns}
:::{.column width="55%"}
- Check that you have the GitHub CLI installed: `gh --version`
...
:::{.column width="45%"}
:::{.callout-tip style="font-size: 26px;"}
`gh` is the convenient route, but a plain `git push` over HTTPS works too, as long as your browser sign-in went through.

What matters is that [some]{.alert} push from this laptop reaches GitHub before the quiz.
:::
...
- Spelled-out options take two dashes: `sort --unique`
- The three you will use often: `-l` long format, `-a` all files including hidden ones, `-h` readable sizes
...
- [Prefer relative paths in anything you share.]{.alert} An absolute path works on exactly one machine
...
### Three tasks inside `~/ds350-review`
...
- Use [two dots]{.alert} to count, [commas]{.alert} to list, and [no spaces]{.alert} inside the braces
:::
:::
:::

## Try it yourself! {#sec-exercise-03}
### More tasks inside `~/ds350-review`
...
- The second lists [four]{.alert}. `input.csv` does not match
...
## For the curious: undoing a bad line

:::{style="margin-top: 30px; font-size: 23px;"}
- Run `>>` twice and the file now holds `brazil` twice
- There is [no undo]{.alert}. Redirection writes, it never removes a line you appended
- Find the line with `grep`, which also prints its number:

```bash
grep -n brazil countries.txt   # lines with "brazil", numbered
```

:::{style="margin-top: 20px;"}
:::

- Then delete it by line number, or by the text it contains
- `sed -i ''` needs the empty `''` on macOS; Linux is just `sed -i`

```bash
sed -i '' '2d' countries.txt         # delete line 2
sed -i '' '/brazil/d' countries.txt  # every "brazil" line
```

:::{style="margin-top: 20px;"}
:::

- You can also drop repeated lines with `uniq`
- `uniq` only collapses lines that repeat [next to each other]{.alert}, not scattered copies

```bash
uniq countries.txt > tmp && mv tmp countries.txt  # drop repeated neighbours
```

:::{style="margin-top: 20px;"}
:::

- Or use a terminal-based editor like `nano` or `vim` to remove the line by hand
:::
...
- [Only amend commits you have not pushed]{.alert} if you're working with others. Otherwise you rewrite history and confuse them
:::
:::
:::
...
:::{style="margin-top: 40px; font-size: 26px;"}
1. Move into `project/` and turn it into a repository
...
git checkout -b feature-x   # older way, same result
git switch main
git merge feature-x
...
:::{.column width="45%"}
- Two people change [lines far apart]{.alert}, and Git merges them for you. The [same line]{.alert}, and it asks you to choose
...
:::{.columns}
:::{.column width="50%"}
The quiz asks for a `commands.txt` holding every command you ran. Reconstructing it from memory never goes well 😅
...
:::{style="margin-top: 20px;"}
[The hacker way]{.alert} 😎
:::

```bash
fc -ln -60 > commands.txt
cat commands.txt
```
:::

:::{.column width="50%"}
- `fc -l` lists the commands you have run, most recent last
- `-60` keeps the last sixty, and `-n` drops the line numbers. Plain `history` on zsh only shows the last 16
- Always `cat` the file before you push. An empty `commands.txt` is the single most common way to lose marks on this quiz
- [Important]{.alert}: keep [one terminal window]{.alert} open for the whole quiz. A new window may not show what you typed in the old one
:::
:::
:::

# Round 6: ready for the quiz {background-color="#1B3A6B"}
...
:::{.column width="45%"}
- It's a good idea to always run `pwd` and `ls` first, so you can see what you are about to remove
...
# Best of luck in the quiz! 🤞 {background-color="#1B3A6B"}
...
:::{style="margin-top: 30px; font-size: 22px;"}
```bash
cd project && git init                        # 1. Git calls your files "untracked"
