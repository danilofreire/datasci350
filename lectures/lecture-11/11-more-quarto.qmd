...
:::{.columns}
:::{.column width="50%"}
- We change gears: [local language models]{.alert}
- How large language models actually work, in enough depth to predict their failures
- A trained model turns out to be [a file you can download]{.alert}, so we take one apart and read every setting inside it
- Then you build a chatbot with a personality of your own choosing
- [Install Ollama before class]{.alert} and run `ollama pull llama3.2:1b`. The download is 1.3 GB, and the classroom wifi cannot do that twenty-five times at once
